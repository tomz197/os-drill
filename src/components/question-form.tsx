      ...
      {facts.map((item, i) => (
        <div
          ...
          className={cn([
            ...
            isSubmitted ? (item.isCorrect ? "hover:bg-green-300" : "hover:bg-red-300") : "hover:bg-gray-200",
          ])}
          onClick={() => {
            if (isSubmitted) return
            if (selectedFacts.includes(item)) {
              setSelectedFacts((prev) => prev.filter((fact) => fact.fact !== item.fact))
            } else {
        ...
        >
          ...
          <div className="flex-1">
            <DisplayFact fact={item.fact} correct={item.correct} isSubmitted={isSubmitted} />
          </div>

          {isSubmitted && selectedFacts.includes(item) && (
            item.isCorrect
              ? <span className="text-green-500"><Check className="w-10 h-10" /></span>
              : <span className="text-red-500"><X className="w-10 h-10" /></span>
          )}
        </div>
      ))}
...
function DisplayFact({ fact, correct, isSubmitted }: { fact: string, correct: string | null, isSubmitted: boolean }) {
